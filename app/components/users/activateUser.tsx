...
const ActivateUser: React.FC<ActivateUserProps> = ({ onUserActive }) => {
    ...
    const handleActivate = async (userId: number) => {
        ...
        try {
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/users/${userId}/status`, { status: 'active' }, { withCredentials: true });
            onUserActive();
            setErrorMessage('');
        } catch (error: any) {
    ...
    return (
        <div>
            <div className='text-sm md:pb-2 text-gray-600 flex items-center justify-between'>
                <div>
                    <h1 className='text-sm'>Activate User</h1>
                    <p className='text-[12px]'>Double check the user before activating it</p>
                </div>
            </div>
            <hr className='-mx-4' />
            ...
            {searchResult && searchResult.data ? (
                <div>
                    ...
                    <button
                        type="button"
                        className="bg-green-500 text-white py-2 px-3 rounded-lg hover:bg-green-600 text-xs"
                        onClick={() => handleActivate(searchResult.data[0].id)}
                    >
                        Activate User
                    </button>
                    {errorMessage && (
